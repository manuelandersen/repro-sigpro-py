...
def ensure_plots_directory() -> str:
    ...
    plots_dir = f"KM_plot_algos_{timestamp}"
...
for nb_it in [10,100,10000]:
    # Inside the Gradient algorithm section, modify the update step:
    with tqdm(total=nb_it, desc=f'Gradiente nb_it {nb_it}') as pbar:
            ...
            alpha_km = 0.5  # Krasnoselskii-Mann parameter
            for it in range(nb_it):
                ...
                operator_eval = zn - gamma_grad * (grad1.flatten() + lambda_val * grad2)
                zn = (1 - alpha_km) * zn + alpha_km * operator_eval
                pbar.update(1)
...
rate_fb2 = []
crit_fb2 = []
# Inside the Forward-backward (2-) section, modify the update step:
with tqdm(total=nb_it, desc='Forward-backward (2-)') as pbar:
        ...
        for it in range(nb_it):
            ...
            operator_eval = tmp - op1.adjoint(1/D1_diag * (op1.direct(tmp) - 
                          prox_huber_l2_matrix(op1.direct(tmp), rhohub, gamma_fb2 * lambda_val * D1_diag)))
            # Krasnoselskii-Mann update
            xn = (1 - alpha_km) * xn + alpha_km * operator_eval
            pbar.update(1)
...
rate_fb3a = []
crit_fb3a = []
# Inside the Forward-backward (3a-) section, modify the update step:
with tqdm(total=nb_it, desc='Forward-backward (3a-)') as pbar:
        ...
        for it in range(nb_it):
            ...
            operator_eval = prox_l2diff(xn - gamma_fb3a * lambda_val * grad2, b, A, gamma_fb3a, Minv)
            # Krasnoselskii-Mann update
            xn = (1 - alpha_km) * xn + alpha_km * operator_eval
            pbar.update(1)
...
rate_fb3b = []
crit_fb3b = []
# Inside the Forward-backward (3b-) section, modify the update step:
with tqdm(total=nb_it, desc='Forward-backward (3b-)') as pbar:
        ...
        for it in range(nb_it):
            ...
            operator_eval = tmp - op2.adjoint(1/D2_diag * (op2.direct(tmp) - 
                          prox_huber_l2_matrix(op2.direct(tmp), rhohub, gamma_fb3b * lambda_val * D2_diag/(1 + gamma_fb3b))))
            # Krasnoselskii-Mann update
            xn = (1 - alpha_km) * xn + alpha_km * operator_eval
            pbar.update(1)
...
rate_pr = []
crit_pr = []
# Inside the Peaceman-Rachford section, modify the update step:
with tqdm(total=nb_it, desc='Peaceman-Rachford') as pbar:
        ...
        for it in range(nb_it):
            ...
            yn = xn - op1.adjoint(1/D1_diag * (op1.direct(xn) - 
                 prox_huber_l2_matrix(op1.direct(xn), rhohub, gamma_pr * lambda_val * D1_diag)))
            tmp = (2 * yn - xn + gamma_pr * b.T) / (1 + gamma_pr)
            zn = tmp - op2.adjoint(1/D2_diag * (op2.direct(tmp) - 
                 prox_huber_l2_matrix(op2.direct(tmp), rhohub, gamma_pr * lambda_val * D2_diag/(1 + gamma_pr))))
            operator_eval = 2 * zn - (2 * yn - xn)
            # Krasnoselskii-Mann update
            xn = (1 - alpha_km) * xn + alpha_km * operator_eval
...
rate_dr = []
crit_dr = []
# Inside the Douglas-Rachford section, modify the update step:
with tqdm(total=nb_it, desc='Douglas-Rachford') as pbar:
        alpha_km = 0.5  # Krasnoselskii-Mann parameter
        for it in range(nb_it):
            rate_dr.append(np.linalg.norm(xn.ravel() - xbardr.ravel()))
            yn = xn - op1.adjoint(1/D1_diag * (op1.direct(xn) - 
                 prox_huber_l2_matrix(op1.direct(xn), rhohub, gamma_dr * lambda_val * D1_diag)))
            tmp = (2 * yn - xn + gamma_dr * b.T) / (1 + gamma_dr)
            zn = tmp - op2.adjoint(1/D2_diag * (op2.direct(tmp) - 
                 prox_huber_l2_matrix(op2.direct(tmp), rhohub, gamma_dr * lambda_val * D2_diag/(1 + gamma_dr))))
            operator_eval = xn + zn - yn
            # Krasnoselskii-Mann update
            xn = (1 - alpha_km) * xn + alpha_km * operator_eval
...
def plot_algorithm_results(results_data, plots_dir):
    ...
    plt.tight_layout()
    plt.savefig(f'{plots_dir}/convergence_rates_km.png', dpi=300, bbox_inches='tight')
    plt.close()
